...
class Runner(object):
  ...
  def run(self, command, environment=None, callbackFunction=None, bufferLimit=52428800):
    ...
    if workloadExecutionLocation:
      return self._remoteRun(remoteSite=workloadExecutionLocation,
                             command=command,
                             environment=environment)

    return self._localRun(command=command,
                          environment=environment,
  ...
  def _remoteRun(self, remoteSite, command, environment=None):
  ...
  def _setUpworkloadCE(self, remoteSite):
    ...
    result = ceFactory.getCE(ceName=workloadCE, ceType=ceType, 
                             ceParametersDict=ceParams)
    if not result['OK']:
