...
class HTCondorResourceUsage(ResourceUsage):
  """
   ...
   However, some Sites have introduced a MaxRuntime variable that sets a wallclock time limit to the allocations and allow
  ...
  def getResourceUsage(self):
    ...
    if len(output) != 2:
      # Output probably empty: cannot open $_CONDOR_JOB_AD? 
      return S_ERROR('Current batch system is not supported')

    wallClockLimit = output[0]
    wallClock = output[1]
    if wallClockLimit == 'undefined':
      # MaxRuntime is not supported
      return S_ERROR('Current batch system is not supported')
